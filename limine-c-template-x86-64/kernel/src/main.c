...
static inline uint8_t inb(uint16_t port) {//chatgpt generated
    uint8_t result;
    asm volatile ("inb %1, %0" : "=a"(result) : "Nd"(port));
    return result;
}

static uint8_t get_key() {//chatgpt generated
...
static key_to_char(uint8_t scancode, char* a) {
...
void kmain(void) {
    ...
    for (;;) {
        uint8_t key = get_key();
        //key != prev_key was to allow another key to be pressed once and get an output while the previous was still down and still have an output instead of having to press the other key twice in order to get its output
        if ((key < 0x80 && mode == 0) || key != prev_key) {//key != prev key may have an issue because prev key was not given a proper value but its value would be 0x00 or something so idk
            //flanterm_write(ft_ctx, "h", 1);
            prev_key = key;
            char c[2];
